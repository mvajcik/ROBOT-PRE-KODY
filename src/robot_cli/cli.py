...
def scan(
    file: Path = typer.Option(
        ..., "--file", "-f", exists=True, readable=True, help="Vstupný Excel"
    ),
    ...
    out_path = (
        out
        or Path("data_out") / f"scan__{file.stem.replace(' ', '_')}__{sheet.replace(' ', '_')}.csv"
    )
    out_path.parent.mkdir(parents=True, exist_ok=True)
    cmd = [
        sys.executable,
        "-m",
        "osm_robot.robot",  # ← bolo "src.osm_robot.robot"
        "--file",
        str(file),
        "--sheet",
        sheet,
