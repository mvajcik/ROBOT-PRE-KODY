...
def _parse_range(a1: str):
    ...
    if r2<r1 or c2<c1: raise ValueError("Range end must be below/right of start")
    return r1,c1,r2,c2
...
def _norm_header(h: str) -> str:
    ...
    x = re.sub(r"\s+","",x)
    xl = x.lower()
    if xl in FULL2SHORT: return FULL2SHORT[xl]
    if re.fullmatch(r"\d{1,2}", x): return "W"+x
    m = re.match(r"^[Ww]0?(\d{1,2})$", x)
    if m: return "W"+str(int(m.group(1)))
    return x
