...
def _sum_range(ws_values, a1_range: str) -> float:
    ...
    for row in ws_values.iter_rows(
        min_row=r1, max_row=r2, min_col=c1, max_col=c2, values_only=True
    ):
        for v in row:
            fv = _to_float(v)
            if fv is not None:
...
# ---------------------------
def _emit_block(
    ws_formula, ws_values, top_left: str, bottom_right: str, sheet_name: str, writer: csv.writer
) -> None:
...
def scan_blocks(
    xlsx_path: str, sheet_name: str, ranges: Iterable[str], out_csv: str, append: bool = False
) -> None:
...
def main() -> None:
    ...
    p.add_argument(
        "--append", action="store_true", help="Append to existing CSV (keep header only once)"
    )
    args = p.parse_args()
